  ...
  if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($db) {
      ...
      if (!$checkflag) {
        ...
        $checkflag->close();
        if($result == 0) {
          $errorMessage = "There is no such puzzle flag. Make sure you have your values correct." . " Flag: " . $flag . " Name: " . $name . " Points: " . $points;
        }else{
          $delflag = $db->prepare('DELETE FROM puzzleflags WHERE flag = ? AND name = ?');
          if (!$delflag) {
            $errorMessage = "Prepare failed 2: (" . $db->errno . ") " . $db->error;
          }else{
            $delflag->bind_param('ss', $flag, $name);
            $delflag->execute();
            $delflag->close();
            $errorMessage = "Puzzle Flag Successfully Deleted!";
          }
        }
      }
        }else{
